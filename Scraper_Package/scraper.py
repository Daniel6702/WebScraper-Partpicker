...
class KomplettHandler(BaseWebsiteHandler):
    ...
    def _get_product_price(self) -> float:
        x = self.request_data.find("span", class_="product-price-now")
        if x is not None:
            return float(x.text.strip(",-").replace(".", ""))
        else:
            return -1.0
...
class ComputerSalgHandler(BaseWebsiteHandler):
    ...
    def _get_product_price(self) -> float:
        data = self.request_data.find("span", itemprop="price")
        if data is not None:
            return float(data.text.strip().replace(".", "").replace(",", "."))
        else:
            return None

    def _get_product_currency(self) -> str:
        data = self.request_data.find("span", itemprop="priceCurrency")
        if data is not None:
